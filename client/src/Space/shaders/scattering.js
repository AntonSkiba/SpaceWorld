export const scattering_shader = (function() {
	...
	void _ComputeScattering(
		...
		float referencePlanetRadius = 2000000.0; //6371000.0;
		float referenceAtmosphereRadius = 200000.0; //100000.0;
		...
		float planetScale = 16.0; // referencePlanetRadius / planetRadius
		float atmosphereScale = scaleRatio / 24.0; // referenceRatio
		float maxDist = distance(worldSpacePos, rayOrigin);
	
		float rayleighScale = 8400.0 / (planetScale * atmosphereScale); // 8500.0
		float mieScale = 1200.0 / (planetScale * atmosphereScale); // 1200.0
		float absorptionHeightMax = 32000.0 * (planetScale * atmosphereScale); // 32000.0
		float absorptionFalloff = 3000.0 / (planetScale * atmosphereScale); // 3000.0
	
		float mu = dot(rayDirection, sunDir);
		float mumu = mu * mu;
		float gg = g * g;
		float phaseRayleigh = 1.0 / (16.0 * PI) * (1.0 + mumu);
		...
		in vec3 sunDir)
	{
		...
		float skyAmt = dot(up, rayDir) * 0.25 + 0.75; // * 0.25 + 0.75
		skyAmt = saturate(skyAmt);
		skyAmt *= skyAmt;
	
		vec3 DARK_BLUE = vec3(0.1, 0.2, 0.3);
		vec3 LIGHT_BLUE = vec3(0.3, 0.4, 0.7);
		...
		in vec3 sunDir)
	{
		...
		float silhouette = saturate((distToPoint - 10000.0) / 10000.0); // 10000.0
		...
		float distFactor = exp(-distToPoint * 0.0005 / (atmosphereThickness)); // 0.0005
		...
		return mix(litColour, blendedColour, scaledDistanceToSurface * 0.25); // 0.25
	}
		...
		in vec3 rayOrigin)
	{
		...
		// чем ближе луна к камере, тем меньше влияния дымки, но с учетом солнца
		vec3 moonDirection = normalize(moonPosition);
		float distToPlayer = 1.0 - length(moonDirection - rayOrigin);
		float distToSun = length(moonDirection - lightDir);
		float coef = 1.0 - distToSun * distToPlayer / 3.0;

		float blendFactor = saturate(coef);

		blendFactor = smoothstep(0.0, 1.0, blendFactor);

		return mix(start, now, blendFactor);
	}
	...
	void main() {
		...
		vec3 scatteringOpacity = vec3(1.0, 1.0, 1.0); // 1.0
		_ComputeScattering(
		...
		diffuse = _ApplyNight(texture2D(tDiffuse, vUv).xyz, diffuse, cameraDirection);
